...
router.post('/makeNewPlayer', async (req, res) => {
  try {
    ...
    player.stat.common.height =
      player.position == '수비수'
        ? Util.makeRandomNumber(205, 175)
        : Util.makeRandomNumber(205, 160)
    player.stat.common.weight =
      player.height >= 180
        ? Util.makeRandomNumber(90, 75)
        : Util.makeRandomNumber(88, 73)
    player.stat.common.foot = Util.randomOfArray(foot, foot.length)

    player.stat.defender.defense =
      player.position == '수비수'
        ? Util.makeRandomNumber(100, 60)
        : Util.makeRandomNumber(65, 1)
    player.stat.defender.mark =
      player.position == '수비수'
        ? Util.makeRandomNumber(100, 40)
        : Util.makeRandomNumber(65, 1)
    player.stat.defender.endurance =
      player.position == '수비수'
        ? Util.makeRandomNumber(100, 40)
        : Util.makeRandomNumber(65, 1)

    player.stat.middle.pass =
      player.position == '미드필더'
        ? Util.makeRandomNumber(100, 60)
        : Util.makeRandomNumber(90, 1)
    player.stat.middle.sight =
      player.position == '미드필더'
        ? Util.makeRandomNumber(100, 40)
        : Util.makeRandomNumber(65, 1)
    player.stat.middle.ballKeep =
      player.position == '미드필더'
        ? Util.makeRandomNumber(100, 40)
        : Util.makeRandomNumber(100, 1)

    player.stat.attack.accelerate =
      player.position == '공격수'
        ? Util.makeRandomNumber(100, 40)
        : Util.makeRandomNumber(70, 1)
    player.stat.attack.scoreAbility =
      player.position == '공격수'
        ? Util.makeRandomNumber(100, 60)
        : Util.makeRandomNumber(70, 1)
    player.stat.attack.shooting =
      player.position == '공격수'
        ? Util.makeRandomNumber(100, 40)
        : Util.makeRandomNumber(75, 1)

    player.stat.goalKeep.block =
      player.position == '골키퍼' ? (100, 60) : Util.makeRandomNumber(30, 1)
    player.stat.goalKeep.throwBall =
      player.position == '골키퍼'
        ? Util.makeRandomNumber(100, 50)
        : Util.makeRandomNumber(50, 1)
    player.stat.goalKeep.communication =
      player.position == '골키퍼'
        ? Util.makeRandomNumber(100, 60)
        : Util.makeRandomNumber(65, 1)
